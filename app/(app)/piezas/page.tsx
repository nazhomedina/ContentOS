...
import { TIPOS, NOMBRE_ESTADO, NOMBRE_TIPO, type EstadoPieza, type Tipo } from "@/lib/dominio/estados";
...
export default async function Piezas({ searchParams }: { searchParams: Promise<{ estado?: string; tipo?: string }> }) {
  const sesion = await sesionActual();
  if (!sesion) redirect("/login");
  const { estado: fEstado, tipo: fTipo } = await searchParams;
  const supabase = await crearClienteServidor();
  const esOwner = sesion.perfil.rol === "owner";

  let q = supabase
    .from("piezas")
    .select("id, id_publico, titulo, tipo, estado, fecha_objetivo, programa_aprobado, notas, created_at, responsable:perfiles!piezas_responsable_id_fkey(nombre)")
  ...
  const { data: todas } = await supabase.from("piezas").select("estado, tipo, programa_aprobado").neq("estado", "archivada");
  ...
  const url = (k: "estado" | "tipo", v: string | null) => {
    const sp = new URLSearchParams();
    const e = k === "estado" ? v : fEstado; const f = k === "tipo" ? v : fTipo;
    if (e) sp.set("estado", e); if (f) sp.set("tipo", f);
  ...
  return (
    <div className="space-y-6">
      <header className="space-y-4">
        ...
        <div className="flex flex-wrap gap-1.5">
          <Chip href={url("tipo", null)} activo={!fTipo} tenue>Todo tipo</Chip>
          {TIPOS.map((f) => (
            <Chip key={f} href={url("tipo", f)} activo={fTipo === f} tenue>{NOMBRE_TIPO[f as Tipo]}</Chip>
          ))}
        </div>
      </header>
              ...
              <li key={p.id}>
                <Link href={`/piezas/${p.id}`} className="flex items-center justify-between gap-3 px-3 py-2.5 hover:bg-muted/50">
                  ...
                  <div className="flex shrink-0 items-center gap-2 text-xs text-muted-foreground">
                    {p.tipo && <InsigniaTipo tipo={p.tipo} />}
