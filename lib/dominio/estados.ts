...
/** Tipo de pieza: el contenedor (reel, carrusel, artículo…). No confundir con formato = Format Card. */
export const TIPOS = ["reel", "yap", "carrusel", "historia", "x", "canal_ig", "newsletter", "articulo", "youtube"] as const;
...
/** Qué sub-etapas de producción aplican a cada tipo. Los demás pasan directo. */
export function subetapas(tipo: string | null): EstadoPieza[] {
  switch (tipo) {
    case "reel":
    case "yap":
    case "youtube":
...
/** Pestañas por tipo del menú: ruta → tipos que agrupa. */
export const PESTANAS_TIPO: { ruta: string; etiqueta: string; tipos: Tipo[]; meta?: string }[] = [
  { ruta: "/reels", etiqueta: "Reels", tipos: ["reel", "yap", "youtube"], meta: "reel" },
  { ruta: "/carruseles", etiqueta: "Carruseles", tipos: ["carrusel"], meta: "carrusel" },
  { ruta: "/articulos", etiqueta: "Artículos", tipos: ["articulo", "x", "canal_ig"], meta: "articulo" },
  { ruta: "/newsletter", etiqueta: "Newsletter", tipos: ["newsletter"], meta: "newsletter" },
];

export function pestanaDeTipo(tipo: string | null): string | null {
  return PESTANAS_TIPO.find((p) => (p.tipos as string[]).includes(tipo ?? ""))?.ruta ?? null;
}
