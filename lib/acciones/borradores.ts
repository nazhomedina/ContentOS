...
import { pestanaDeTipo } from "@/lib/dominio/estados";
...
/** Un borrador entra a producción: se le da tipo y pasa a redacción. */
export async function pasarARedaccion(piezaId: string, tipo: string): Promise<Resultado & { ruta?: string }> {
  if (!tipo) return { ok: false, mensaje: "Elige el tipo para producirla." };
  const supabase = await crearClienteServidor();
  const { error: e1 } = await supabase.from("piezas").update({ tipo }).eq("id", piezaId);
  ...
  revalidar();
  return { ok: true, mensaje: "En redacción. Claude escribe contenido e hipótesis desde aquí.", ruta: pestanaDeTipo(tipo) ?? "/piezas" };
}
